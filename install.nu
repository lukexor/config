...
let brew_packages = [
  ...
  sdl2_mixer sdl2_ttf sqlite starship stow tmux tree vim watchman
  wget
]
let apt_packages = [
  bash cc65 cmake coreutils docker fzf git hexedit llvm lolcat mongocli neovim
  nodejs openjdk-11-jdk openssl postgresql prettier python python3 libsdl2-2.0-0
  libsdl2-gfx-1.0-0 libsdl2-image-2.0-0 libsdl2-mixer-2.0-0 libsdl2-ttf-2.0-0
  sqlite starship stow tmux tree vim watchman wget
]
let npm_packages = [eslint_d]
let cargo_packages = [
  cargo-asm cargo-count cargo-expand cargo-flamegraph cargo-generate
  cargo-outdated cargo-readme cargo-tree cargo-watch flamegraph neovide ripgrep
  wasm-pack
]
