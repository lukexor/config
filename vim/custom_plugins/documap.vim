...
command! -nargs=+ Map call <SID>Define('Map', <q-args>, 1)
command! -nargs=+ Noremap call <SID>Define('Noremap', <q-args>, 1)
command! -nargs=+ Nmap call <SID>Define('Nmap', <q-args>, 1)
command! -nargs=+ Nnoremap call <SID>Define('Nnoremap', <q-args>, 1)
command! -nargs=+ Imap call <SID>Define('Imap', <q-args>, 1)
command! -nargs=+ Inoremap call <SID>Define('Inoremap', <q-args>, 1)
command! -nargs=+ Xmap call <SID>Define('Xmap', <q-args>, 1)
...
function! s:Define (type, args, map)
    ...
    try
        let [match, opts, iopts, lhs, desc, rhs; etc] = matchlist(a:args, s:mapdoc_syntax)
    ...
    " echom 'Mapping type:"' . tolower(a:type) . '" opts:"' . opts . '" lhs:"' . lhs . '" rhs:"' . rhs . '"'
...
Nmap <localleader>h [Print this list] :call <SID>ListMappings()<CR>
