...
export DJANGOROOT=/usr/lib/python2.7/site-packages

# CHANGE: to admin bin scripts directory under Django.
# This should work for the svn version.
export DJANGOBIN=$DJANGOROOT/django/bin
pathadd $DJANGOBIN

# CHANGE: to TCP port for testing.
# You go to http://localhost:8000 to hit the test sever.
export DJANGOTESTPORT=8000

# CHANGE: to base directory where you keep your Django projects.
export DJANGOPROJECTBASE=$HOME/dev/django

# CHANGE: to default current Django project
export DJANGOCURRENTPROJECT=web
export DJANGO_SETTINGS="$DJANGOPROJECTBASE/$DJANGOCURRENTPROJECT"
...
function dsrc() { cd $DJANGOPROJECTBASE }

function dls() { ls $DJANGOPROJECTBASE }

function dadmin() { django-admin.py $@ }
...
function dproj() {
    if [ $# -eq 0 ]; then
        if [ $DJANGOCURRENTPROJECT ]; then
            echo "Current project is: $DJANGOCURRENTPROJECT"
        else
            echo "Run 'dproj <project>' first"
        fi
    else
        if [ -d $DJANGOPROJECTBASE/$1 ]; then
            export DJANGOCURRENTPROJECT=$1
            export DJANGO_SETTINGS="$DJANGOPROJECTBASE/$DJANGOCURRENTPROJECT"
            echo "Current project set to $1 located in $DJANGOPROJECTBASE."
        else
...
function dapp() { dcheck && python manage.py startapp $@ }

function dcd() { dcheck }

function dsync() { dcheck && python manage.py syncdb && popd }

function drun() { dcheck && python manage.py runserver $DJANGOTESTPORT$* && popd }
